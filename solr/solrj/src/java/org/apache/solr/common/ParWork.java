...
//  };
  ...
  public static Future<?> submit(String threadName, Runnable task) {
    ...
    //log.info("submit CLASSLOADER={}", Thread.currentThread().getClass().getClassLoader());
...
//      executor.allowCoreThreadTimeOut(true);
  ...
  public static ExecutorService getMyPerThreadExecutor() {
    ...
    if (service == null) {
      ...
      return getExecutorService("ParWorkPerThread", 32, true);
    }
  ...
  public static ExecutorService getExecutorService(String name, int maximumPoolSize, boolean wait) {
    ...
    return new VirtualExecutorService(name, getRootSharedExecutor(), maximumPoolSize, wait);
  }
  ...
  private void handleObject(AtomicReference<Throwable> exception, final TimeTracker workUnitTracker, Object ob) {
    ...
    try {
      ...
      if (ignoreExceptions) {
        warns.add(t);
        log.error("Error handling close for an object: " + object.getClass().getSimpleName(), new ObjectReleaseTrackerTestImpl.ObjectTrackerException(t));
