  ...
  private void register(String coreName, final CoreDescriptor desc, boolean afterExpiration) {
   ...
   // LeaderElector leaderElector = null;
    ...
    try {
      ...
      zkStateReader.waitForState(collection, 5000, TimeUnit.MILLISECONDS, (n, c) -> {
        if (log.isDebugEnabled()) {
        ...
        if (fleader != null && fleader.getState() == Replica.State.ACTIVE) {
          ...
          ParWork.getRootSharedIOExecutor().submit(() -> finishRegistration(afterExpiration, coreName, desc, fleader.getName()));
          return true;
        }
...
//      }
   ...
   */
  public Replica getLeaderProps(final String collection, int id,
   ...
   * @throws SessionExpiredException on zk session expiration.
   */
  public Replica getLeaderProps(final String collection, int id, final String slice, int timeoutms, boolean failImmediatelyOnExpiration)
      throws InterruptedException, SessionExpiredException { // MRM TODO: look at failImmediatelyOnExpiration

    try {
      getZkStateReader().waitForState(collection, timeoutms, TimeUnit.SECONDS, (n, c) -> c != null && c.getLeader(slice, n) != null);

      byte[] data = zkClient.getData(ZkStateReader.getShardLeadersPath(collection, slice), null, null);
      ZkCoreNodeProps leaderProps = new ZkCoreNodeProps(ZkNodeProps.load(data));

      return new Replica(leaderProps.getNodeProps().getStr(CORE_NAME_PROP), leaderProps.getNodeProps().getProperties(), collection, id, null);

    } catch (Exception e) {
      log.error("Exception getting leader props", e);
      throw new SolrException(ErrorCode.SERVER_ERROR, e);
    }
  ...
  private void joinElection(CoreDescriptor cd, boolean joinAtHead) {
    ...
    Replica replica = new Replica(cd.getName(), props, collection, Integer.parseInt(collId), null);
...
//      }
  ...
  private void unregisterConfListener(String confDir, Runnable listener) {
    ...
    confDirectoryListeners.computeIfPresent(confDir, (c, confListeners) -> {
