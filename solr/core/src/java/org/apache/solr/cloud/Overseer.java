...
//  private volatile ExecutorService taskExecutor;
  ...
  public void start(Integer id, ElectionContext context, boolean weAreReplacement) throws KeeperException {
    ...
    try {
      ...
      collectionQueueExecutor = ParWork.getParExecutorService("collectionQueueExecutor", 16, 32, 1000);
          // ParWork.getExecutorService("OverseerCollectionQueueTaskExecutor", 128, false);

      zkWriterExecutor = ParWork.getParExecutorService("zkWriterExecutor", 16, 32, 1000);
     ...
     // zkWriterExecutor.prestartAllCoreThreads();
      overseerOnlyClient = new Http2SolrClient.Builder().markInternalRequest().maxThreadPoolSize(3024).idleTimeout(600000).connectionTimeout(5000).build();
