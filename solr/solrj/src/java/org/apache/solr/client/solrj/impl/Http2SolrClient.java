...
public class Http2SolrClient extends SolrClient {
  ...
  protected Http2SolrClient(String serverBaseUrl, Builder builder) {
    ...
    int maxOutstandingAsyncRequests = 1500;
  ...
  private HttpClient createHttpClient(Builder builder) {
    ...
    int minThreads = Integer.getInteger("solr.minHttp2ClientThreads", 32);
    ...
    QueuedThreadPool httpClientExecutor = new QueuedThreadPool(maxThreads, minThreads);
    try {
    ...
    try {
      ...
      InputStreamResponseListener isl = new InputStreamResponseListener() {
        ...
        public void onHeaders(Response response) {
          ...
          ParWork.submit("Http2SolrClientAsync", () -> {
            InputStream is = null;
            try {
              is =  getInputStream();
              NamedList<Object> body = processErrorsAndResponse(solrRequest, parser, is, response);

              asyncListener.onSuccess(body);
            } catch (Exception e) {
              if (SolrException.getRootCause(e) != CANCELLED_EXCEPTION  &&
                  response.getStatus() != ErrorCode.CANCEL_STREAM_ERROR.code) { // I don't know that i means it was a sucess though...) {
...
//      }
  ...
  public static class Builder {
    ...
    public static int DEFAULT_MAX_THREADS = Integer.getInteger("solr.maxHttp2ClientThreads", 512);
  ...
  private static SimpleResponse doDelete(String url, Map<String,String> headers)
    ...
    try (Http2SolrClient http2SolrClient = new Http2SolrClient.Builder().build()) {
      Request req = http2SolrClient.getHttpClient().newRequest(url).method(DELETE);
