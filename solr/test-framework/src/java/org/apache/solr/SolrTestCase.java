...
public class SolrTestCase extends Assert {
  ...
  public static TestRule solrClassRules =
      RuleChain.outerRule(classEnvRule = new TestRuleSetupAndRestoreClassEnv()).around(new SystemPropertiesRestoreRule())
          .around(suiteFailureMarker = new TestRuleMarkFailure()).around(new NoClassHooksShadowingRule())
          .around(tempFilesCleanupRule = new TestRuleTemporaryFilesCleanup(suiteFailureMarker)).around(new RevertDefaultThreadHandlerRule());
...
//  })
  ...
  public static void beforeSolrTestCase() throws Exception {
    ...
    System.setProperty("solr.tests.ramPerThreadHardLimitMB", "20");
...
//      System.setProperty("solr.recoveryThrottle", "1000");
     ...
     // System.setProperty("lucene.cms.override_core_count", "3");
      ...
      System.setProperty("solr.rootSharedThreadPoolMaxSize", "512");
...
//      }
  ...
  private static boolean interrupt(Thread thread, String... nameContains) {
    ...
    if (thread.getName().contains("pool-") || thread.getName().startsWith(ParWork.ROOT_EXEC_NAME)) {
      return false;
    }
    ...
    if (state == Thread.State.WAITING) { // TMP
      thread.interrupt();
      return true;
    }
