...
public class XmlConfigFile { // formerly simply "Config"
  ...
  public static TinyDocumentImpl parseXml(SolrResourceLoader loader, InputSource is, Properties substituteProps) throws XPathException {
   ...
   // saxSrc.setXMLReader(loader.getXmlReader());
    ...
    net.sf.saxon.Configuration config = loader.getConf();
    TreeInfo saxTreeInfo = config.buildDocumentTree(saxSrc, po);
    return (TinyDocumentImpl) saxTreeInfo.getRootNode();
...
//    }
  ...
  public Object evaluate(TinyDocumentImpl tree, String path, QName type) {
    try {
     //  log.error("COMPILE XPATH FOR PATH OR MANUALLY GRAPH THESE TINY NODES = {} QNAME={}", path, type);
      String xstr = normalize(path);
