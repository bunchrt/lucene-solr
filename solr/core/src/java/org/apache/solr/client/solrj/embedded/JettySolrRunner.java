...
public class JettySolrRunner implements Closeable {
  ...
  private final QueuedThreadPool qtp;
...
//    }
    ...
    if (config.onlyHttp1) {
      ...
      connector = new ServerConnector(server, null, scheduler, null, 32, 32, new SslConnectionFactory(sslContextFactory, http1ConnectionFactory.getProtocol()),
...
//    server.setDumpBeforeStop(true);
    ...
    HandlerWrapper chain;
    {
     ...
     final ServletContextHandler root = new ServletContextHandler(server, config.context, ServletContextHandler.NO_SESSIONS);
      ...
      //              root.addFilter(entry.getKey(), entry.getValue(), EnumSet.of(DispatcherType.REQUEST, DispatcherType.ASYNC));
      //            }
      //
...
//          }
      ...
      //    SolrPaths.ensureUserFilesDataDir(solrHomePath);
          ...
          ParWork.getRootSharedExecutor().submit(()-> SolrDispatchFilter.setupJvmMetrics(cores));
