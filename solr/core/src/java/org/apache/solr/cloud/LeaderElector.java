...
public class LeaderElector implements Closeable {
  ...
  private boolean checkIfIamLeader(final ElectionContext context, boolean replacement) throws InterruptedException {
    try {
      ...
      if (!seqs.contains(leaderSeqNodeName)) {
        log.debug("Our node is no longer in line to be leader");

        return true;
      }

      if (log.isDebugEnabled()) {
        log.debug("The leader election node is {}", leaderSeqNodeName);
      }

      if (leaderSeqNodeName.equals(seqs.get(0))) {

        // I am the leader
        if (log.isDebugEnabled()) log.debug("I am the potential leader {}, running leader process", context.replica.getName());
      ...
      log.debug("already closed", e);
      return false;
    } catch (KeeperException.SessionExpiredException e) {
      log.warn("session expired", e);
      return false;
    } catch (Exception e) {
      log.error("Failed in checkIfIamLeader", e);
      return true;
  ...
  protected boolean runIamLeaderProcess(final ElectionContext context, boolean weAreReplacement) throws KeeperException, InterruptedException, IOException {
    ...
    try {
      ...
      log.warn("Failed becoming leader {}", context.replica);
      return false;
    } finally {
      if (!success) {
        leadersFailed.inc();
  ...
  public void doJoinElection(ElectionContext context, boolean replacement, boolean joinAtHead) throws KeeperException, InterruptedException {
    ...
    while (true) {
      try {
        ...
        log.error("No leader election node found", e);
        break;
      }
    }
