    ...
    if (iw != null) {
      if (!rollback) {
        try {
          ...
          if (!(TestInjection.injectSkipIndexWriterCommitOnClose(iw) || Boolean.getBoolean("solr.skipCommitOnClose"))) {
            iw.commit();
          }
          iw.close();
        } catch (Exception e) {
...
//        }
    ...
    try {
      ...
      recoveryFuture = corecontainer.getUpdateShardHandler().getRecoveryExecutor().submit(recoveryTask);
      success = true;
