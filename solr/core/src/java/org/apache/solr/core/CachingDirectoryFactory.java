...
public abstract class CachingDirectoryFactory extends DirectoryFactory {
  ...
  protected final Map<String, Directory> byPathCache = new ConcurrentHashMap<>();
  ...
  public final Directory get(String path, DirContext dirContext, String rawLockType)
    ...
    return byPathCache.computeIfAbsent(fullPath, dir -> {
      try {
        return create(fullPath, createLockFactory(rawLockType), dirContext);
      } catch (IOException e) {
...
//      } else {
  ...
  public void remove(String path, boolean deleteAfterCoreClose) throws IOException {
    ...
    Directory dir = byPathCache.remove(normalize(path));
    org.apache.solr.common.util.IOUtils.closeQuietly(dir);
  }
